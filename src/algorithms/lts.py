...
def _lts_step(
    ...
    dm_inv_new, f_new, mu_hat_new = lts_update(
        carry.design_matrix_inv, carry.cumulative_reward_context, contexts_t[action], reward,
    )
    new_carry = LtsCarry(
...
def make_lts_step_fn(context_dim: int, true_theta: jnp.ndarray, **kwargs) -> Callable:
    ...
    **kwargs
    ...
    def step_fn(carry: LtsCarry, x: tuple) -> tuple:
        ...
        return _lts_step(carry, contexts_t, true_theta, noise_t, v)
